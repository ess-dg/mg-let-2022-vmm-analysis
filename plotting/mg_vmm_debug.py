    ...
    for ring_id in range(number_rings):
        for vmm_id in range(number_vmms_per_fen):
            ...
            if df_vmm.shape[0] > 0:
                ...
                ax.hist2d([-1000], [-1000], bins=[64, 1024],
                          norm=LogNorm(vmin=1, vmax=max_v),
                          range=[[-0.5, 63.5], [-0.5, 1022.5]],
                          cmap='jet')
                fig.colorbar(h[3], ax=ax)
...
def plot_phs_wires_vs_grids(df, title, number_bins=200):
    def plot_phs_wires_vs_grids_column(df_column, max_v, ax):
        # Histogram data
        hist, xbins, ybins, im = ax.hist2d(df_column.wadc, df_column.gadc_sum, bins=[number_bins, number_bins],
                                           norm=LogNorm(vmin=1, vmax=max_v),
...
def plot_phs_wires_and_grids(df, title, number_bins=100):
    def plot_phs_wires_and_grids_column(df_column, ax, number_bins):
        # Histogram data
        ax.hist(df_column.wadc, bins=number_bins, range=[-0.5, 1023.5], label='Wires', color='blue', alpha=1, histtype='step')
        ax.hist(df_column.gadc_sum, bins=number_bins, range=[-0.5, 1023.5], label='Grids', color='red', alpha=1, histtype='step')
        ax.set_yscale('log')
        ax.set_xlabel("ADC")
